...
Для запуска проекта в окружении, максимально приближенном к продакшн (включая сканер-микросервис), используйте Docker.

1. **Установите Docker и Docker Compose.**

2. **Настройте переменные окружения для Docker:**
   Создайте файл `.env.local` (если еще не создан) и добавьте ваши ключи Supabase:
   ```env
   NEXT_PUBLIC_SUPABASE_URL=your-project-url
   NEXT_PUBLIC_SUPABASE_ANON_KEY=your-anon-key
   SCANNER_SECRET=my-super-secret-key # Можете использовать любое значение
   ```
...
3. **Запустите Docker Compose:**
   ...
   ```
   Эта команда соберет образы и запустит все сервисы. Приложение будет доступно по адресу [http://localhost:3000](http://localhost:3000).
